...
### 5. 📊 토익 예상 점수 매핑 피드백 (TOEIC Score Estimation)
- 국내 영어 평가 환경에서 직관적인 지표로 활용할 수 있도록, CEFR 단계 및 정답 개수별 토익 예상 점수를 제공합니다.
  - **초급(Beginner) 합격 시**: 320 ~ 600점대 수준 (정답 수 7~10개 기준)
  - **중급(Intermediate) 합격 시**: 600 ~ 860점대 수준 (정답 수 7~10개 기준)
  - **고급(Advanced) 합격 시**: 860 ~ 990점(만점) 수준 (정답 수 7~10개 기준)
- 이 결과는 각 스테이지 합격 통과 화면 및 최종 인증서(수료증) 상단에 배지 및 점수 텍스트 형식으로 연동되어 노출됩니다.
